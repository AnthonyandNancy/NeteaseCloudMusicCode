  ...
  <div class="songsheet">
    <div class="head">
        ...
        <el-dropdown placement="top">
          ...
          <el-dropdown-menu slot="dropdown">
            <div style="width: 80vh; height: 40vh">
              <el-button
                style="width: 100%"
                @click="handalChooseSongSheet('全部')"
              >全部歌单</el-button>
              <div
                v-for="(item, index) in catlistPlaylist.categories"
                :key="index"
                class="catlistPlaylist"
              >
                <p class="text-danger" style="text-align: left">{{ item }}</p>
                <ul>
                  <li
                    v-for="(subitem, subindex) in catlistPlaylist.sub"
                    v-if="subitem.category == index"
                    :key="subindex"
                    class="list-unstyled subitem"
                    @click="handalChooseSongSheet(subitem.name)"
                  >
                    {{ subitem.name }}
                  </li>
                </ul>
      ...
      <ul class="hotPlayListUl">
        <li
          v-for="(item, index) in hotPlayList.tags"
          :key="index"
          class="list-unstyled hotPlayListLi"
          :class="{ isLiactive: hotLi == index }"
          @click="handalPlayHotList(index, item.name)"
        >
          {{ item.name }}
        </li>
      </ul>
    ...
    <div class="palyLists">
      <div
        ...
        :key="index"
        class="sheet"
        @click="handalGoPlayListInfo(item.id)"
      >
        <img
